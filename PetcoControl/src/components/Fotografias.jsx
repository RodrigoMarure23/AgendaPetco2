  ...
  if (empleados.length > 0) {
    return (
      <div>
        ...
        <div className='fotosContenedor'>
          {
            empleados.map((empleado, index) => (
              <div key={index} >
                <img className={empleado.imagen ? "newPhoto" : "backGroundDefault"} onClick={(e) => {
                  empleado.imagen ? null : setMostrarNewFoto(!mostrarNewFoto)
                  setNombre2(empleado.nombr)
                  e.preventDefault()
                  console.log("NombreEmpleado: ", empleado.nombr)
                }} src={empleado.nombr ? empleado.imagen : ""} height={"100px"} width={"100px"} />
        ...
        <button className='BotonCerrarNewPhoto btn2' style={{ display: mostrarNewFoto ? "" : "none" }} onClick={(e) => {
          setMostrarNewFoto(!mostrarNewFoto)
          e.preventDefault()
        }
        }>cerrar</button>
      </div>
